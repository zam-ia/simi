...
export async function GET(_request: Request, { params }: { params: { orderId: string } }) {
  try {
    ...
    let { data: order, error } = await supabase.from("orders").select(fullSelect).eq("id", params.orderId).maybeSingle();
    if (error && isMissingPhaseTwoColumn(error)) {
      const fallback = await supabase.from("orders").select(baseSelect).eq("id", params.orderId).maybeSingle();
      order = fallback.data;
    ...
    const [{ data: items }, { data: events }] = await Promise.all([
      ...
      supabase.from("order_status_events").select("*").eq("order_id", params.orderId).order("created_at", { ascending: true })
    ]);
